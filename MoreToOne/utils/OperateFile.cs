...
namespace CashMachine.utils
{
    ...
    class OperateFile
    {
        ...
        public static long GetFileLength(string filePath)
        {
            ...
            {
                MessageBox.Show("获取文件字节大小失败，因为文件不存在!", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
        ...
        public static string GetFileVersionNumber(string filePath)
        {
            if (!File.Exists(filePath))
            {
                MessageBox.Show("获取文件版本号失败，因为文件不存在!", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                return null;
            }
            else
            {
        ...
        public static long GetFileLastWriteTime(string filePath)
        {
            if (!File.Exists(filePath))
            {
                MessageBox.Show("获取文件最后一次修改日期失败，因为文件不存在!", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                return 0;
            }
            else
            {
        ...
        public static byte[] FiletoByte(string filePath)
        {
            ...
            if (!File.Exists(filePath))
            {
                MessageBox.Show("将文件转化成byte[]数组失败，因为文件不存在!", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                return null;
            }
            else
            {
