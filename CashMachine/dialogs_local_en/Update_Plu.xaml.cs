...
{
    /// <summary>
    /// Update_Plu.xaml 的交互逻辑
    /// </summary>
    public partial class Update_Plu : Window
    {
        //声明一个变量
        Plu plu = new Plu();
        //声明一个变量
        ...
        //有参构造函数
        public Update_Plu(Plu plu, string dataBase, string dataBase_TAX, string dataBase_PRO)
        {
            this.plu = plu;//赋值
        ...
        public void Init()
        {
            textBlock2_8_1.Text = this.plu.Number;
            textBox2_8_1.Text = this.plu.Name;
            textBox2_8_2.Text = this.plu.Barcode;
            textBox2_8_3.Text = Convert.ToString(this.plu.Price);
            if (this.plu.Stock_Control == 1)
            {
                checkBox2_8_1.IsChecked = true;
            }
            textBox2_8_4.Text = Convert.ToString(this.plu.Stock_Amount);
            textBlock2_8_4.Text = this.plu.Tax_Index.ToString();
            if (this.plu.Tax_Code.Equals("B"))
            {
                textBlock2_8_5.Text = "*";
                textBlock2_8_7.Text = "*";
                textBlock2_8_6.Visibility = Visibility.Visible;
                textBox2_8_5.Visibility = Visibility.Visible;
                textBox2_8_5.Text = this.plu.RRP.ToString();
            }
        }
        /// <summary>
        /// barcode 只允许输入0-9数字
        ...
        private void textBox2_8_2_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                //判断结果
                if (Convert.ToInt32(dt.Rows[0]["COUNTS"]) == 0 || textBox2_8_2.Text == this.plu.Barcode)//若数量为0
                {
        ...
        private void button2_8_1_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("The Tax Index is required, please click on the 'Choose' buttom!", "Information", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            else if (textBlock2_8_4.Text.Equals("0"))
            {
                MessageBox.Show("The Tax Index can not be 0 !", "Information", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            else if (textBox2_8_3.Text.Equals("."))
            {
            ...
            else
            {
                ...
                plu.Used = "1";
                //SQL语句
                string sql = "UPDATE Goods_Info SET Name=@Name,Barcode=@Barcode,Price=@Price,RRP=@RRP,Tax_Index=@Tax_Index,"
                    + "Stock_Amount=@Stock_Amount,Stock_Control=@Stock_Control,Currency=@Currency,Used=@Used WHERE Number=@Number";
                //配置SQL语句里的参数
